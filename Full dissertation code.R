...
hayle_biplot <- fviz_pca_var(hayle_metal_pca, col.var = "contrib",
                             gradient.cols = c("cadetblue", "goldenrod", "tomato"),
                             repel = TRUE) +
...
quantile(plot_df$pH, probs = c(0, 1/3, 2/3, 1), na.rm = TRUE)
#       0% 33.33333% 66.66667%      100% 
#     4.92      6.46      6.97      7.71 

quantile(plot_df$Temp, probs = c(0, 1/3, 2/3, 1), na.rm = TRUE)
#       0% 33.33333% 66.66667%      100% 
#      7.9      11.5      13.1      16.2 

quantile(plot_df$Altitude, probs = c(0, 1/3, 2/3, 1), na.rm = TRUE)
#       0% 33.33333% 66.66667%      100% 
#        4        40       103       152 
...
top15_table <- data %>%
  group_by(Catchment, Phylum) %>%
  summarise(total = sum(Count, na.rm = TRUE), .groups = "drop") %>%
  group_by(Catchment) %>%
  slice_max(total, n = 15, with_ties = FALSE) %>%
  arrange(Catchment, desc(total))

top15_table

colours <- c("Acidobacteriota"   = "sienna4",
            ...
            "Nitrospirota"      = "deeppink3", 
            "Deinococcota"      = "darkslategray",
            "Other"             = "grey")
...
rel_abund_carnon_top <- rel_abund_carnon %>%
  mutate(Phylum = ifelse(Phylum %in% top15_carnon, Phylum, "Other"))
...
rel_abund_cober_top <- rel_abund_cober %>%
  mutate(Phylum = ifelse(Phylum %in% top15_cober, Phylum, "Other"))
...
###################################################
######## TAXONOMY: Genus relative abundance #####
###################################################
...
genus_data <- genus_data %>%
  filter(!str_detect(tolower(Genus), "unclassified|uncultured"))

data_Hayle_genus <- genus_data %>% filter(Catchment == "Hayle")
data_Red_genus <- genus_data %>% filter(Catchment == "Red")
data_Carnon_genus <- genus_data %>% filter(Catchment == "Carnon")
data_Cober_genus <- genus_data %>% filter(Catchment == "Cober")

data_Hayle2_genus <- data_Hayle_genus %>% group_by(Location, Genus, Month, Altitude) %>% 
  summarise(count_mean = mean(Count))

data_Red2_genus <- data_Red_genus %>% group_by(Location, Genus, Month, Altitude) %>% 
  summarise(count_mean = mean(Count))

data_Carnon2_genus <- data_Carnon_genus %>% group_by(Location, Genus, Month, Altitude) %>% 
  summarise(count_mean = mean(Count))

data_Cober2_genus <- data_Cober_genus %>% group_by(Location, Genus, Month, Altitude) %>% 
  summarise(count_mean = mean(Count))

genus_colours <- c("Bacillus" = "brown1", 
                   "Chloroplast" = "green4", 
                   "Clostridium_sensu_stricto_1" = "cyan4", 
                   "Flavobacterium" = "blueviolet", 
                   "Rhodoferax" = "orchid1", 
                   "Romboutsia" = "goldenrod3", 
                   "Mycobacterium" = "aquamarine", 
                   "Paeniclostridium" = "coral", 
                   "hgcI_clade" = "maroon2", 
                   "Acinetobacter" = "sienna", 
                   "Desulfosporosinus" = "seagreen", 
                   "Gallionella" = "darkgreen", 
                   "Paenibacillus" = "yellow2", 
                   "Polynucleobacter" = "violetred4", 
                   "Pseudarcicella" = "cornflowerblue", 
                   "Turicibacter" = "palegreen", 
                   "Bacteroides" = "tomato3", 
                   "Blautia" = "tan2", 
                   "Candidatus_Rhabdochlamydia" = "palevioletred1", 
                   "Intestinibacter" = "orange4", 
                   "Novosphingobium" = "lavender", 
                   "Rickettsiella" = "salmon", 
                   "C39" = "magenta4", 
                   "Candidatus_Methylopumilus" = "khaki", 
                   "Pseudarcobacter" = "lightgoldenrod4", 
                   "Aquabacterium" = "darkolivegreen3", 
                   "Pseudomonas" = "darkslateblue", 
                   "Sulfuricurvum" = "darkslategrey", 
                   "Clostridium_sensu_stricto_9" = "lightsteelblue",
                   "Prevotella" = "bisque", 
                   "Other" = "lightgrey")

## Relative abundance per sample
rel_abund_hayle_genus <- data_Hayle2_genus  %>%
  group_by(Location, Month) %>%
  mutate(rel_abund = count_mean / sum(count_mean)) %>%
  ungroup() %>%
  mutate(Location = fct_reorder(Location, Altitude, .desc = T))
...
## Top 15 phyla in HAYLE 
top15_hayle_genus <- data_Hayle2_genus %>%
  group_by(Genus) %>%
  summarise(total = sum(count_mean, na.rm = T), .groups = "drop") %>%
  slice_max(total, n = 15) %>%
  pull(Genus)
...
## Recode others as "Other"
rel_abund_hayle_top_genus <- rel_abund_hayle_genus %>%
  mutate(Genus = ifelse(Genus %in% top15_hayle_genus, Genus, "Other"))

rel_abund_hayle_top_genus <- rel_abund_hayle_top_genus %>%
  group_by(Location, Month) %>%
  mutate(Genus = fct_reorder(Genus, rel_abund, .desc = T)) %>%
  ungroup() 

ggplot(rel_abund_hayle_top_genus, aes(x = Location, y = rel_abund, fill = Genus)) +
  geom_col() +
  facet_wrap(~ Month, scales = "free_x") +
  labs(x = "Sample (high → low altitude)", y = "Relative abundance", fill = "Genus") +
  theme_bw() +
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()) +
...
## Relative abundance per sample
rel_abund_red_genus <- data_Red2_genus %>%
  group_by(Month, Location) %>%
  mutate(rel_abund = count_mean / sum(count_mean)) %>%
  ungroup() %>%
  mutate(Location = fct_reorder(Location, Altitude, .desc = T))
...
## Top 15 genus in Red
top15_red_genus <- data_Red2_genus %>%
  group_by(Genus) %>%
  summarise(total = sum(count_mean, na.rm = T), .groups = "drop") %>%
  slice_max(total, n = 15) %>%
  pull(Genus)
...
## Recode others as "Other"
rel_abund_red_top_genus <- rel_abund_red_genus %>%
  mutate(Genus = ifelse(Genus %in% top15_red_genus, Genus, "Other"))

rel_abund_red_top_genus <- rel_abund_red_top_genus %>%
  group_by(Location, Month) %>%
  mutate(Genus = fct_reorder(Genus, rel_abund)) %>%
  ungroup() %>%
  mutate(Genus = fct_reorder(Genus, rel_abund, .desc = T))

## Plot
ggplot(rel_abund_red_top_genus, aes(x = Location, y = rel_abund, fill = Genus)) +
  geom_col() +
  facet_wrap(~ Month, scales = "free_x") +
  labs(x = "Sample (high → low altitude)", y = "Relative abundance", fill = "Genus") +
  theme_bw() +
...
## Relative abundance per sample
rel_abund_carnon_genus <- data_Carnon2_genus %>%
  group_by(Month, Location) %>%
  mutate(rel_abund = count_mean / sum(count_mean)) %>%
  ungroup() %>%
  mutate(Location = fct_reorder(Location, Altitude, .desc = T))

## Top 15 genus in Carnon
top15_carnon_genus <- data_Carnon2_genus %>%
  group_by(Genus) %>%
  summarise(total = sum(count_mean, na.rm = T), .groups = "drop") %>%
  slice_max(total, n = 15) %>%
  pull(Genus)
...
## Recode others as "Other"
rel_abund_carnon_top_genus <- rel_abund_carnon_genus %>%
  mutate(Genus = ifelse(Genus %in% top15_carnon_genus, Genus, "Other"))

rel_abund_carnon_top_genus <- rel_abund_carnon_top_genus %>%
  group_by(Location, Month) %>%
  mutate(Genus = fct_reorder(Genus, rel_abund)) %>%
  ungroup() %>%
  mutate(Genus = fct_reorder(Genus, rel_abund, .desc = T))
...
## Plot
ggplot(rel_abund_carnon_top_genus, aes(x = Location, y = rel_abund, fill = Genus)) +
  geom_col() +
  facet_wrap(~ Month, scales = "free_x") +
  labs(x = "Sample (high → low altitude)", y = "Relative abundance", fill = "Genus") +
  theme_bw() +
...
## Relative abundance per sample
rel_abund_cober_genus <- data_Cober2_genus %>%
  group_by(Month, Location) %>%
  mutate(rel_abund = count_mean / sum(count_mean)) %>%
  ungroup() %>%
  mutate(Location = fct_reorder(Location, Altitude, .desc = T))
...
## Top 15 genus in Cober
top15_cober_genus <- data_Cober2_genus %>%
  group_by(Genus) %>%
  summarise(total = sum(count_mean, na.rm = T), .groups = "drop") %>%
  slice_max(total, n = 15) %>%
  pull(Genus)
...
## Recode others as "Other"
rel_abund_cober_top_genus <- rel_abund_cober_genus %>%
  mutate(Genus = ifelse(Genus %in% top15_cober_genus, Genus, "Other"))

rel_abund_cober_top_genus <- rel_abund_cober_top_genus %>%
  group_by(Location, Month) %>%
  mutate(Genus = fct_reorder(Genus, rel_abund)) %>%
  ungroup() %>%
  mutate(Genus = fct_reorder(Genus, rel_abund, .desc = T))
...
## Plot
ggplot(rel_abund_cober_top_genus, aes(x = Location, y = rel_abund, fill = Genus)) +
  geom_col() +
  facet_wrap(~ Month, scales = "free_x") +
  labs(x = "Sample (high → low altitude)", y = "Relative abundance", fill = "Genus") +
  theme_bw() +
